	...
	<?php else : // this is displayed if there are no comments so far ?>
		...
		<?php if ('open' == $post->comment_status) : ?>
			...
			<div id="comments">
				...
				<p><?php _e('Não há comentários', 'woothemes') ?></p>
...
<div id="respond">
	...
	<h3><?php comment_form_title( __('Deixe um comentário', 'woothemes'), __('Leave a Reply to %s', 'woothemes') ); ?></h3>
	...
	<?php else : //No registration required ?>
		...
		<?php else : //If user is not logged in ?>
			...
			<p>
				...
				<label for="author"><?php _e('Nome', 'woothemes') ?> <?php if ($req) echo "(obrigatório)"; ?></label>
			</p>

			<p>
				<input type="text" name="email" class="txt" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" />
				<label for="email"><?php _e('E-mail', 'woothemes') ?> <?php if ($req) ?> (<?php _e('obrigatório', 'woothemes'); ?>) <?php ; ?></label>
			</p>
		...
		<input name="submit" type="submit" id="submit" tabindex="5" value="<?php _e('Inserir comentário', 'woothemes') ?>" />
